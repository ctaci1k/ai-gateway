...
Адмін бачить адмін-панель: список усіх акаунтів, створення/редагування юзерів, квоти, статистику.

- **Хто стає адміном:** автоматично — той, чий username збігається з `ADMIN_USERNAME` (за замовч. `admin`)
  при реєстрації. Або вручну (нижче). Або через саму панель (адмін може зробити іншого адміном).
- **Поточний адмін:** `stas` (зроблений адміном напряму в БД).

**Зробити користувача адміном напряму в БД** (підстав username):
```bash
docker exec ai-gateway-db-1 psql -U ai_gateway -d ai_gateway -c \
  "UPDATE users SET is_admin=true, max_requests_per_minute=NULL, max_requests_per_day=NULL WHERE username='ІМЯ';"
```
> Після зміни — вийти з акаунту і зайти знову (права читаються при вході).

**Подивитись усіх юзерів і хто адмін:**
```bash
docker exec ai-gateway-db-1 psql -U ai_gateway -d ai_gateway -tAc "SELECT id, username, is_admin FROM users;"
```

**Створити окремий адмін-акаунт:** найпростіше — зареєструватись на сайті, тоді підняти його до адміна
командою вище; або створити через адмін-панель (кнопка створення користувача).

---

## 8. ВІДКАТ на попередню версію (якщо нова зламана)
