...
## Ревізія PH29.2 — неповний рядок блокує «Зберегти» (не тихий «успіх»)

> Власник: на вбудованому слоті (AI 1/2/3 / Суддя) можна **обрати нетиповий
> endpoint** (напр. OpenAI) і натиснути «Зберегти», **не ввівши ключ+модель** — і
> воно рапортує «Збережено», хоча вибір тихо викидається. Це баг: такий рядок
> **не має зберігатися** — Save має **впасти з явним повідомленням** «впишіть
> API-ключ і ID моделі», лишивши рядок як є (endpoint не зникає).
>
> **Рішення:** «Зберегти» спершу шукає **неповні рядки** (щось заповнене, але нема
> ключ+модель; для кастомних — нема endpoint+ключ+модель). Якщо такі є — **не
> валідувати, не персистити, не показувати «Збережено»**, а підсвітити рядки
> червоним із повідомленням і лишити їх без змін (endpoint не бланкувати). Пусті
> вбудовані слоти (геть порожні) — це норма (вбудований ключ застосунку), не блок.

- [x] **F1. Чисті хелпери** (`store/KeysContext.tsx`) — експортовано
  `isBuiltinIncomplete` / `isCustomIncomplete` / `findIncompleteSlots(state)`
  (slot-id неповних рядків, judge=JUDGE_SLOT). Дублікати з `KeysForm` прибрано
  (імпортує ці). + юніт-тести (5 кейсів).
- [x] **F2. Блокування в `KeysForm.save()`** — після прибирання геть порожніх
  кастомних: якщо `findIncompleteSlots` непорожній → `setSubmittedIncomplete(true)`,
  **return до валідації/персисту**, без «Збережено», рядки лишено. Скид
  `submittedIncomplete` при редагуванні (`updateJudge`/`updateResponder`).
- [x] **F3. Повідомлення/підсвітка** — неповний рядок: `keys-hint` (живо)
  ескалюється до `keys-error` (червоне) при заблокованому Save; порожні
  ключ/модель → `keys-input--invalid`. Тексти — наявні
  `keys.incompleteBuiltin`/`keys.incompleteCustom`.
- [x] **F4. Гейти+доки+деплой** — tsc/eslint/prettier/**vitest(34)**/build зелені;
  `docs/08`/`docs/10` нотатка; `status: done`; коміт+push.
...
- Ревізія **PH29.2**: неповний рядок (обрано endpoint без ключа+моделі тощо)
  тепер **блокує «Зберегти»** — без тихого «успіху»: червоне повідомлення на рядку
  + підсвітка порожніх полів, рядок лишається. Хелпери винесено в `KeysContext`
  (`findIncompleteSlots` + тести). Гейти tsc/eslint/prettier/vitest(34)/build.
