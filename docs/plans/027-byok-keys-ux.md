...
> Власник на демо: (1) вбудовані endpoint'и не мають бути у списку — до вбудованого
> лише **скидання**; (2) «Очистити» має бути й на AI 1/2/3, не лише в судді;
> (3) «Власний…» (вільний текст) прибрати — лише вибір зі списку; (4) не виводити
> «Вбудовані» як пункти/підпис; (5) рядок із base URL, але без ключа+моделі **не
> має зберігатися**. Архітектор обрав **Design X**: у вбудованих слотах (Суддя +
> AI 1/2/3) base URL прибрано зовсім (endpoint фіксований), лишилися ключ+модель+
> «Очистити»; base URL (вибір зі списку, required) — **лише в кастомних AI 4/5**.
> **Свідомо прибрано:** PH22-оверайд endpoint'у дефолтних слотів і base URL судді
> з UI (для чужого провайдера — AI 4/5; «суддя на своєму провайдері» — окремий план).
...
- [x] **E2. `BaseUrlSelect`** — чистий `<select>` (каталог + disabled-плейсхолдер
  «Оберіть ендпоінт…»); прибрано optgroup, default-опцію, текстове поле/customMode;
  fallback-опція для невідомого (legacy) значення. Лише для кастомних слотів.
- [x] **E3. `KeysForm`** — суддя + AI 1/2/3: прибрано base URL, лишилися ключ+модель
  (+ⓘ) і кнопка **«Очистити»** (`keys.clear`) на всіх трьох + суддя; кастомні
  AI 4/5: `BaseUrlSelect`(required)+ключ+модель+ⓘ, **«Видалити»**. Inline-підказка
  для неповних рядків (`keys.incompleteBuiltin`/`keys.incompleteCustom`).
- [x] **E4. `KeysContext.buildPersistedState`** — неповний built-in слот/суддя
  (нема ключа+моделі) бланкується назад до вбудованого; дефолтні слоти не
  персистять override `baseUrl`; кастомний неповний не валідується/не зберігається.
  + юніт-тест (blanks half-filled built-in).
- [x] **E5. i18n** — прибрано мертві (`useDefaultEndpoint*`, `baseUrlBuiltin`,
  ...
  `keys.incompleteBuiltin`, `keys.incompleteCustom`; паритет uk/pl/en (vitest ✓).
- [x] **E6. CSS+гейти+доки+деплой** — `.keys-hint` на токенах; гейти
  tsc/eslint/prettier/**vitest(28)**/build зелені; `docs/06`/`docs/08`(PH29.1)/
...
- Останній виконаний крок: **E6 (PH29.1)** — ревізія за фідбеком власника
  (Design X): base URL прибрано з вбудованих слотів+судді, «Очистити» на AI 1/2/3,
  каталог без built-in/без «Власний…», заборона збереження неповних рядків. Гейти
  tsc/eslint/prettier/vitest(28)/build зелені; доки 06/08/10; `status: done`;
  коміт+push.
