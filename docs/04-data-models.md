...
- `usage_events` (PH15, D-10; **PH27/D-18**) — `id` (PK), `user_id` (FK→users, CASCADE, index), `created_at` (index), `mode` (`"compare"`/`"single"`), `message`, `selected_model` (nullable), `total_tokens` (int, nullable), `success` (bool), **`chat_id`** (FK→chats, **ondelete SET NULL**, nullable, index), **`billable`** (bool, NOT NULL, server_default `true`), **`token_estimated`** (bool, NOT NULL, server_default `false`), **`key_fingerprint`** (String(32), nullable; **PH31/D-21**), **`model_name`** (String(128), nullable; **PH32/D-22**), **`judge_model_name`** (String(128), nullable; **PH34/D-24**), **`judge_key_fingerprint`** (String(32), nullable; **PH34/D-24**). **Append-only канонічний per-turn ledger** (на відміну від rolling `interactions`, **не обрізається**): джерело правди для enforcement квоти (лічба за вікна хвилина/доба), admin-перегляду і **Звітів (PH27)**. Один рядок = один хід (Compare-хід = 1 подія). **PH27 (D-18):** пишуться **ВСІ** ходи, у т.ч. BYOK; `billable=false` → хід на власному ключі (квота не списувалась). **Квотні вікна рахують лише `billable=true`** — повнота ledger не змінює поведінки лімітів. `chat_id` зв'язує подію зі збереженим чатом (видалення чату лишає аудит, відв'язує → група «видалені/ad-hoc»). `total_tokens` — реальні (provider `usage`) або **оцінка** (`token_estimated=true`, евристика `ceil(chars/4)`, `core/tokens.py`). **PH31 (D-21):** `key_fingerprint` — display-only маска BYOK-ключа моделі-переможця ходу (`перші4••••останні4`, напр. `gsk_••••OTzu`); `NULL` = вбудований (app) ключ. Денормалізується з розшифрованого `ByokConfig` у момент запису (`memory/byok_repository.key_fingerprint`); **плейнтекст ключа тут ніколи не зберігається**. Звіти розділяють ту саму модель за `(selected_model, key_fingerprint)`. **PH32 (D-22):** `model_name` — **реальна** модель, що відповіла/перемогла (денормалізовано в момент запису): Compare — `all_responses[selected_model]["model"]`, Single — модель зі стріму. `selected_model` **лишається слотом** (стабільний групувальник: квоти/персоналізація/судова логіка не чіпаються), а `model_name` несе правду, коли власний ключ на вбудованому слоті ходить іншою моделлю (напр. слот `groq` → `gpt-4o`). `NULL` для історичних рядків → FE фолбек на лейбл слота; **плейнтекст ключа тут ніколи не зберігається** (це лише ідентифікатор моделі). Звіти додають `model_name` третім виміром групування «за моделлю». **PH34 (D-24, B9b):** `judge_model_name` + `judge_key_fingerprint` — денормалізований **доданий (BYOK) суддя** Compare-ходу, заповнюються **лише** коли суддя на власному ключі (вбудований суддя → обидва `NULL`, не показуємо); `judge_key_fingerprint` — та сама display-only маска (плейнтекст тут ніколи не зберігається). Звіти синтезують **похідний** judge-рядок (`by_model` `role="judge"`, токени=0 — не подвоюють переможця) і judge-вузол під «own» (`breakdown`); журнал/CSV несуть judge-колонки inline. **Рядок переможця лишається канонічним one-per-turn; квоти/`billable` — без змін.** Міграції Alembic `0005_quotas_usage` (поля `users` + таблиця), `0007_usage_ledger` (3 колонки ledger), `0009_usage_key_fingerprint` (колонка маски), `0010_usage_model_name` (реальна модель) та `0011_usage_judge_byok` (2 judge-колонки).
