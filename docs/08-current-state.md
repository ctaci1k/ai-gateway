...
↪ **Ревізія PH29.1 (фідбек власника, Design X = максимальне спрощення):** вбудовані слоти **не повинні пропонувати вибір endpoint'у** — до вбудованого лише скидання. Тому: **base URL прибрано з UI у судді й у дефолтних AI 1/2/3** (endpoint фіксований), лишилися ключ+ID моделі + кнопка **«Очистити»** (тепер і на AI 1/2/3, не лише в судді). **base URL лишився тільки в кастомних AI 4/5** — `BaseUrlSelect` спрощено до чистого `<select>` лише з каталогу (`BYOK_BASE_URLS` тепер **лише 10 сумісних провайдерів — без built-in, без груп, без «Власний…»/вільного тексту**; disabled-плейсхолдер «Оберіть ендпоінт…»; fallback-опція для legacy). **Неповний рядок більше не зберігається:** `buildPersistedState` бланкує half-filled built-in/суддю назад до вбудованого, дефолтні слоти не персистять override `baseUrl`; кастомні неповні не валідуються; обидва дають inline-підказку. i18n: прибрано мертві ключі, додано `keys.baseUrlChoose`/`keys.incompleteBuiltin`/`keys.incompleteCustom`. **Свідомо прибрано** PH22-оверайд endpoint'у дефолтних слотів і base URL судді з UI (`JudgeKey.baseUrl` лишається в payload-контракті; бекенд приймає). Гейти: FE tsc/eslint/prettier/**vitest(28)**/build; бекенд без змін.
