...
- **Корінь:** `selected_model`/`chats.model` зберігають **id слота**, а FE-мапа `RESPONDER_LABELS` жорстко мапить слот→назву. Коли власний ключ на вбудованому слоті ходить **іншою** моделлю (напр. слот `groq` → `gpt-4o`), звіти/адмін/банер переможця Compare/картки replay показували **неправдиву** назву слота.
- **Принцип:** чітко розділено **slot** (маршрутна ідентичність — лишається групувальником) і **`model`** (реальна модель). Кожен запис ходу став **самоописним**: несе реальну модель + ознаку джерела ключа.
- **БД:** нова колонка `usage_events.model_name` (String(128), nullable; Alembic `0010_usage_model_name`, `batch_alter_table`). Денормалізується в момент запису: Compare — `all_responses[selected_model]["model"]`; Single — модель зі стріму. `selected_model` **лишається слотом** (Single вирівняно під Compare: `selected_model=request.provider`, реальна модель → `model_name`; старі Single-рядки не бекфіляться — задокументована межа). `NULL` для історичних рядків → FE фолбек на лейбл слота.
- **Самоописний хід (`is_byok`).** `_safe_generate` обчислює `is_byok = isinstance(provider, TransientProvider)` і повертає `is_byok` + реальну `model` в **обох** гілках (успіх/невдача); `execute_many` прокидає їх у кожен `all_responses[slot]`, `failed_providers[*]` (+`model`) і `execution_metadata[*]`. Схеми `ProviderResponse`/`FailedProvider`/`ExecutionMetadataItem` + `is_byok`. `build_interaction_record` несе їх у `chat_messages.payload` без зміни схеми → **replay** Compare (банер + картки) показує правду **без поточних ключів**.
- **Звіти + адмін.** `by_model` групує за `(selected_model, key_fingerprint, model_name)`; `breakdown` ключує вузол моделі трійкою + несе `model_name`; `by_chat` — `func.max(model_name)`; `events`/`iter_events_for_csv` + CSV-стовпець `model_name` (після `model`). Схеми `ModelUsage`/`BreakdownModel`/`ChatUsage`/`UsageEventDetail` + `model_name`; адмін `UsageEventOut` + `model_name` (`admin_service.user_usage`).
- **Frontend.** Єдиний хелпер `utils/models.modelDisplay(slot, model, isByok)` + `reportUtils.reportModel(row)`. Звіти (`ByModel`/`Breakdown`/`Activity` → `reportModel`; `ByChat` → `model_name ?? modelLabel(slot)`); банер `SelectorBanner` бере `winnerModel`/`winnerIsByok` зі збереженого ходу (`CompareTurn`); картки `CompareColumn`/`CompareFailedCard` — `modelDisplay(...)` зі збереженого ходу (прибрано залежність `useKeys`); `AdminPanel` — `ev.model_name ?? responderLabel(ev.selected_model)`. Типи `types/api.ts`. **Свідома межа (не люк):** композерні/active-model поверхні (`MainHead`/`SingleModelPicker`/`ModelSwitcher`/`sidebarStatus`) лишаються на `byokModelId` (поточні ключі) — показують, ЧИМ слатимеш зараз.
- **Безпека/межі:** плейнтекст ключа й KEK — ніколи в логах/відповідях/ledger; шифрування БД (D-20) не чіпане; квоти/`access`-фільтр/orchestrator-логіка вибору/`selected_model`-семантика — без змін. Нового env/owner-action нема.
- **Гейти:** BE **196 passed** + ruff/black; FE tsc/eslint/prettier/vitest(35)/build; i18n паритет (нових текстів нема).
...
- **(A) Крипто + сховище.** `core/secret_box.py` — AES-256-GCM **envelope**: KEK з env `BYOK_ENCRYPTION_KEY` (base64, 32 байти), випадковий 96-біт nonce на запис, **AAD = `f"{user_id}:{slot}"`** (підміна рядка в БД провалює GCM-тег), `key_version` під ротацію. Плейнтекст ключа й KEK **ніколи** в БД/логах. Модель `db/models.py::ByokCredential` (`user_id` FK CASCADE, `slot`, `base_url?`, `model_id`, `key_ciphertext`/`key_nonce` LargeBinary, `key_last4`, `key_version`, `custom`; UNIQUE(`user_id`,`slot`)) + Alembic `0008_byok_credentials`. Репозиторій `memory/byok_repository.py` (per-user, `list_metadata`/`resolve_key`/`load_config`/`upsert`/`delete`; decrypt **лише тут**).
- **(B) Ендпоінти + чат із БД.** `routes/keys.py`: `GET /keys` (write-only метадані: `slot`/`base_url`/`model_id`/`last4`/`custom` — повний ключ **не** віддається), `PUT /keys` (валідує кожен entry живим викликом конкурентно, пише **лише робочі** зашифровано; переюзає збережений ключ при зміні model/base_url), `DELETE /keys/{slot}`. `routes/chat.py` тепер вантажить BYOK з БД (`ByokRepository(user.id).load_config()`); тіло `ChatRequest.byok` **ігнорується** (deprecated — клієнт не може інжектити ключі). Квота/orchestrator/reports **не чіпані** (`all_byok`/`should_charge` працюють як раніше).
- **(C) Frontend write-only.** `services/keysApi.ts` (`getKeys`/`putKeys`/`deleteKey`); `store/KeysContext.tsx` переписано на серверну метадата-модель (гідрація через `getKeys()` per-account; logout чистить лише локальний кеш; `byokPayload` прибрано). `components/keys/KeysForm.tsx` — write-only (маска `••••last4`, prefill model/base, Save шле лише змінені рядки). Транзит ключів прибрано з `useCompare`/`ComposerContext`/`chatApi`/`compareApi`; `sidebarStatus` рахує з метаданих. Усі здобутки PH29.x збережено.
- **(D) Model discovery.** `POST /keys/models` → `provider.list_models()` (`GET {base_url}/models`), фолбек `{models:[], error_reason}`. `core/model_filter.py::is_chat_model` (евристика; повний список + прапор `is_chat`). FE `components/keys/ModelCombobox.tsx` — нативний `<input list>`+`<datalist>` (ручний фолбек безкоштовно), «Завантажити моделі», стани loading/empty/error, chat-фільтр + «показати всі», session-кеш. Discovery листає і за збереженим ключем.
- **(E) Довідник ключів (без лого).** `utils/byokEndpoints.ts` розширено (`keysUrl`/`modelsUrl`/`needsKey` на провайдера + `BUILTIN_PROVIDER_LINKS` + хелпери). `components/keys/ProviderGuide.tsx` — глобальна кнопка «Де взяти ключі та моделі?» (a11y-панель). `InfoTip` отримав `links`; біля поля ключа — контекстний «Отримати API-ключ ↗» (built-in за слотом, custom за base URL). Усі лінки `target="_blank" rel="noopener noreferrer"`; жодні дані назовні.
- **Безпека:** «не логуємо ключі» (D-12) збережено; «не в БД» замінено на «зашифровано в БД» (D-20). **Owner-action:** `BYOK_ENCRYPTION_KEY` має бути в прод-секретах (інакше API відмовляє у збереженні/використанні ключів), той самий у всіх середовищах однієї БД.
- **Гейти:** BE **178 passed** + ruff/black; FE tsc/eslint/prettier/**vitest(29)**/build; i18n паритет uk/pl/en (+12 ключів: маска/discovery/довідник).

↪ **Ревізія PH30.2 (фідбек власника — вибір моделі замість ручного вводу + тексти):** (1) **Поле моделі — `<select>` (тільки вибір)**, не вільний ввід: `ModelCombobox` переписано на `<select>` зі знайдених моделей (chat-фільтр + «показати всі»; поточне значення завжди як опція). (2) **Запобіжник «ніколи не блокувати» збережено через розумний фолбек за кодом помилки** `/models`: `services/provider_service.py::classify_discovery_failure(error)` розрізняє **404→`no_models`** (провайдер без `/models`, напр. Perplexity → відкривається ручний ввід), **401/403→`bad_key`** (поганий ключ → ручне НЕ відкривається, показ «перевірте ключ»), 429→`rate_limited`, timeout/мережа. Порожній список при валідному ключі (200, `[]`) теж відкриває ручний ввід. Тобто поганий ключ більше не «розблоковує» вільний ввід. (3) **Ревізія текстів `keys.*`** під реальність PH30/PH30.1: виправлено `keys.info.apiKey` («ніколи не зберігається» → «зберігається зашифровано»), `keys.intro`/`keys.modelsFound`/`keys.incomplete*` (модель **вибирається**), `keys.info.modelId` («Завантажити моделі»). Бекенд-контракт `/keys/models` без змін (лише точніший `error_reason`). Гейти: BE pytest(**19 byok**)+ruff/black; FE tsc/eslint/prettier/vitest(29)/build; i18n паритет 334.
- **(PH30.2a) Прибрано Ollama** зі списку Base URL і довідника: для **хостованого** застосунку `http://localhost:11434` вказувало б на localhost **сервера** (де Ollama нема), а не машини користувача — нефункціональний/оманливий пункт. Тепер каталог = **9 OpenAI-сумісних провайдерів** (+ 3 вбудовані Groq/Cerebras/SambaNova). Дані — `utils/byokEndpoints.ts`.

↪ **Ревізія PH30.1 (фідбек власника — усунення нестиковок UX):** (1) `keys.intro` лишався зі **старим** текстом «ключі лише в цій вкладці браузера», що суперечив `keys.noteSecurity` (серверне шифрування) — переписано (uk/pl/en). (2) Слоти **підписано провайдером**: «AI 1 · Groq», «AI 2 · Cerebras», «AI 3 · SambaNova» (`keys.responderSlotNamed`), щоб видно було, куди йде свій ключ. (3) **Реверс частини PH29.1:** вбудовані провайдери (Groq/Cerebras/SambaNova) тепер **у випадайці Base URL** — у кожному слоті його **власний** провайдер = іменований дефолт (`keys.endpointNamed` «Groq · типовий ендпоінт»), а **два інші вбудовані + сумісні** показуються як override-опції (`selectableEndpointsForSlot`); список збігається з довідником «Де взяти ключі». `utils/byokEndpoints.ts` отримав `BUILTIN_BASE_URLS` (єдине джерело base-URL+links вбудованих) + хелпери `builtinForSlot`/`selectableEndpointsForSlot`; `BaseUrlSelect` приймає `options`+named-`placeholder`. Бекенд не змінювався (вже приймав будь-який `base_url` на слот). Гейти: FE tsc/eslint/prettier/vitest(29)/build.
...
↪ **Ревізія PH29.1 (фідбек власника, Варіант A):** вбудовані endpoint'и не мають бути **у списку** (до вбудованого лише скидання), вільний текст прибрати, неповний рядок не зберігати — але **base URL лишається на ВСІХ рядках як SELECT**. Тому: **base URL — `BaseUrlSelect` (чистий `<select>`) на Судді + AI 1/2/3 + кастомних**, з каталогу `BYOK_BASE_URLS` = **лише 10 сумісних провайдерів** (без вбудованих у списку, без груп, без «Власний…»/вільного тексту; disabled-плейсхолдер через prop; fallback-опція для legacy). **Вбудовані слоти** (Суддя + AI 1/2/3): base URL **необов'язковий** — порожньо = вбудований endpoint, плейсхолдер «Типовий ендпоінт»; вибір сумісного = override; **«Очистити»** (тепер і на AI 1/2/3) скидає назад до вбудованого. **Кастомні AI 4/5**: base URL обов'язковий + «Видалити». **Неповний рядок не зберігається:** `buildPersistedState` бланкує half-filled built-in/суддю до вбудованого (навіть якщо є лише base URL); повний built-in слот зберігає свій override; кастомні неповні не валідуються; усі дають inline-підказку. i18n: прибрано мертві ключі, додано `keys.baseUrlChoose`/`keys.endpointDefault`/`keys.incompleteBuiltin`/`keys.incompleteCustom`. **Свідомо прибрано** вільний текст base URL і вбудовані провайдери зі списку. Гейти: FE tsc/eslint/prettier/**vitest(29)**/build; бекенд без змін.

↪ **Ревізія PH29.2 (фідбек власника):** неповний рядок більше не дає тихого «успіху». На вбудованому слоті можна було обрати endpoint (напр. OpenAI) без ключа+моделі й натиснути «Зберегти» — рапортувало «Збережено», а вибір тихо викидався. Тепер **«Зберегти» блокується**, якщо будь-який рядок частково заповнений (щось є, але нема ключ+модель; для кастомних — нема endpoint+ключ+модель): не валідується, не персиститься, без «Збережено» — рядок підсвічується **червоним** із повідомленням (`keys.incompleteBuiltin`/`keys.incompleteCustom`), порожні поля ключ/модель позначаються `keys-input--invalid`, рядок лишається як є (endpoint не зникає). Чисті хелпери `isBuiltinIncomplete`/`isCustomIncomplete`/`findIncompleteSlots` винесено в `store/KeysContext` (+5 юніт-тестів). Гейти: FE tsc/eslint/prettier/**vitest(34)**/build; бекенд без змін.
