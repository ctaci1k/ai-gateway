...
- `GET /reports/by-model` → `200 { models: [{ model|null, key_fingerprint|null, requests, total_tokens, successful }] }` (за спаданням запитів). **PH31 (D-21):** рядки розділено за `(model, key_fingerprint)` — та сама модель окремо як вбудована (`key_fingerprint=null`) і як кожен власний ключ (маска `перші4••••останні4`).
...
- `GET /reports/breakdown` (PH28) → `200 { groups: [{ access_key:"app"|"own", requests, total_tokens, models: [{ model|null, key_fingerprint|null, requests, total_tokens, chats: [{ chat_id|null, title|null, mode|null, requests, total_tokens }] }] }] }`. Вкладене дерево **ключ доступу → модель → чати** для акордеон-drill-down; зібране в Python з однієї пласкої вибірки (LEFT JOIN chats), сортування за запитами спадно; `chat_id=null` — bucket «видалені/ad-hoc». **PH31 (D-21):** вузол моделі ключований парою `(model, key_fingerprint)` — та сама модель built-in vs власний ключ → окремі вузли; верхній рівень `access_key` (billable) без змін.
- `GET /reports/events?cursor=&limit=` → `200 { events: [{ id, created_at, mode, model|null, total_tokens|null, token_estimated, success, billable, key_fingerprint|null, message, chat_id|null, chat_title|null }], next_cursor|null }`. Keyset-пагінація за `(created_at,id)` спадно; `limit` 1..200 (дефолт 50); `cursor` = `"<iso>|<id>"`. `key_fingerprint` (PH31/D-21) = маска BYOK-ключа ходу або `null` (вбудована).
- `GET /reports/events.csv` → `200 text/csv` (`Content-Disposition: attachment`), стрім рядками з того самого вікна; колонки: `created_at, mode, model, key_fingerprint, chat_title, billable, total_tokens, token_estimated, success, message` (PH31/D-21 додає `key_fingerprint` після `model`). CSV-екранування — стандартним `csv`-модулем; великі обсяги не тримаються в памʼяті (keyset-сторінки).
