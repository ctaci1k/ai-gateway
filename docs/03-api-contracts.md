...
**Request:** `{ "message": "...", "provider": "groq", "rag_enabled": false, "byok": null }`
- `provider` — обрана модель Single (`groq` / `cerebras` / `sambanova`, або BYOK-слот/слот судді — NQ6); UI має перемикач (PH13/B1, розширено PH17).
