...
- `GET /reports/by-model` → `200 { models: [{ model|null, key_fingerprint|null, model_name|null, requests, total_tokens, successful }] }` (за спаданням запитів). **PH31 (D-21):** рядки розділено за `(model, key_fingerprint)`. **PH32 (D-22):** групування включає й `model_name` (реальна модель) — той самий слот з власним ключем на різні реальні моделі → окремі рядки; `model_name=null` → FE показує лейбл слота (`model`=слот лишається стабільним групувальником).
