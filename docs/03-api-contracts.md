...
**Захищені (потрібні session; мутації — ще й CSRF):** `/chat`, `/chat/stream`, `/chat/structured`, `/keys/validate` (PH17), `/keys` (GET/PUT/DELETE, PH30), `/keys/models` (PH30), `/chats` (CRUD), `/documents` (CRUD), `/preferences`, `/preferences/manual-selection`, `/memory` (GET/DELETE), `/memory/json`, `/admin/*` (ще й `is_admin`). Дані **ізольовані per-user**. Публічні: `/`, `/providers`, `/providers/info`.
...
```jsonc
{
  ...
  "history": [],                       // PH40/P3: транзитна памʼять контексту чату — [{role:"user"|"assistant", content}]; відповідач бачить попередні ходи (Compare-хід асистента = переможна відповідь). Бекенд клампить до останніх 10 ходів і обрізає кожне до max_message_length; новий чат = []; RAG/мова — лише на поточне питання; суддя НЕ отримує історію
  "locale": null                       // PH33/B3b: UI-локаль (uk/pl/en) — фолбек-мова відповіді, коли мова повідомлення неоднозначна; відповідач відповідає мовою повідомлення
  // "byok": {...}                      // PH17 → DEPRECATED (PH30, D-20): поле приймається, але ІГНОРУЄТЬСЯ —
  //                                    // ключі вантажаться зі зашифрованого серверного сховища (/keys CRUD)
}
```
...
```jsonc
{
  ...
  "all_responses": {
    "groq":      { "response": "...", "model": "llama-3.3-70b-versatile", "execution_time": 1.2, "provider": "groq", "success": true, "is_byok": false },
    "cerebras":  { "...": "..." },
    "sambanova": { "...": "..." }
  },
  "failed_providers": [ { "provider": "x", "error": "...", "reason": "rate_limited", "model": "gpt-4o", "is_byok": true } ],
  "execution_metadata": [ { "provider": "groq", "success": true, "execution_time": 1.2, "model": "...", "error": null, "is_byok": false } ],
  // PH32 (D-22): кожна відповідь у all_responses + кожен failed_provider несуть is_byok (true=власний ключ) + реальну model → хід самоописний (replay Compare показує правдиву модель без поточних ключів)
...
- `locale` (PH33/B3b) → інжектиться як директива мови у промт **відповідача** (Single і Compare): відповідач відповідає мовою повідомлення користувача, а `locale` — фолбек, коли мова неоднозначна. Суддя/ембеддинги/квоти не зачіпаються; `null`/невідоме → фолбек EN.
...
- **BYOK (PH17 → PH30, D-12 → D-20):** з PH30 ключі вантажаться зі **зашифрованого серверного сховища** per-account (`byok_credentials`), **не** з тіла запиту. Поле `byok` у тілі лишається опційним, але **ігнорується** (deprecated; клієнт не може інжектити ключі). Сервер будує ті самі **транзитні** провайдери (judge + responders) із розшифрованих ключів (merge з реєстром для незаданих слотів; 3–5 відповідачів). **Квота при BYOK:** Compare рахується як 1, **крім** випадку, коли **всі** учасники (усі відповідачі + суддя) на своїх ключах → безліміт (не enforced, не пишеться `usage_events`). Ключі **ніколи не логуються**; у БД — лише шифротекст (AES-256-GCM envelope). Керування ключами — `/keys` CRUD (нижче).

---

## POST `/chat/stream` — Single (стрімінг)

**Request:** `{ "message": "...", "provider": "groq", "rag_enabled": false, "chat_id": null, "history": [] }`
- `history` (PH40/P3) — транзитна памʼять контексту чату: `[{role:"user"|"assistant", content}]`. Обрана модель бачить попередні ходи. Бекенд клампить (останні 10 ходів, обрізка до `max_message_length`); новий чат = `[]`; RAG/мова застосовуються лише до поточного питання.
...
- **BYOK (PH30, D-20):** ключ обраної моделі вантажиться зі **серверного сховища** (поле `byok` у тілі ігнорується). **Квота:** Single списується, лише якщо обрана модель — на **дефолтному** ключі; на своєму (BYOK) ключі — **безкоштовно** (не enforced, не пишеться `usage_events`).
...
## `/keys` CRUD — серверне зашифроване сховище BYOK (PH30, D-20)

Потрібні session; мутації (`PUT`/`DELETE`) — ще й CSRF. Per-user ізоляція по `user_id`. Ключі зберігаються **зашифровано** (AES-256-GCM envelope, `core/secret_box.py`); плейнтекст **ніколи** не повертається й не логується.

- **`GET /keys`** → write-only метадані поточного юзера: `{ "keys": [ { "slot": "groq", "base_url": "", "model_id": "my-llama", "last4": "1234", "custom": false } ] }`. Повний ключ **не віддається** (лише `last4`).
- **`PUT /keys`** — `{ "entries": [ { "slot": "groq", "model_id": "...", "api_key": "...", "base_url": null, "custom": false } ] }`. Для кожного entry — живий валід-виклик (конкурентно); **робочі** зберігаються зашифровано, **невдалі** повертають `ok:false` без збереження. `api_key` **опційний**: якщо відсутній на наявному слоті — переюзається збережений ключ (змінюються лише `model_id`/`base_url`). Відповідь: `{ "results": [ { "slot", "ok", "error" } ], "keys": [<свіжі метадані>] }`. Якщо `BYOK_ENCRYPTION_KEY` не заданий — усі entry `ok:false` («Key storage is not configured»).
- **`DELETE /keys/{slot}`** → видаляє слот, повертає `{ "keys": [<решта метаданих>] }` (кнопки «Очистити»/«Видалити» у UI).

## POST `/keys/models` — model discovery (PH30, D-20)

Потрібні session + CSRF. Резолвить ендпоінт+ключ і викликає `GET {base_url}/models` провайдера **через сервер** (CORS+секретність). **Request:** `{ "slot": "groq", "base_url": null, "api_key": null }`. `api_key` опційний: якщо відсутній — переюзається **збережений** ключ слота (decrypt server-side). **Response (200):** `{ "models": [ { "id": "gpt-4o", "is_chat": true }, { "id": "text-embedding-3-small", "is_chat": false } ], "error_reason": null }`. На помилці → `{ "models": [], "error_reason": "..." }`. **Коди класифікуються точно (PH30.2, `classify_discovery_failure`) за HTTP-статусом**, щоб FE розрізняв випадки: **`no_models`** (404 — провайдер без `/models`, напр. Perplexity) і **порожній список при валідному ключі** (200, `[]`, `error_reason:null`) → FE відкриває **ручний ввід**; **`bad_key`** (401/403 — поганий ключ) → FE лишає тільки вибір + «перевірте ключ» (ручне **не** розблоковується); `rate_limited` (429), `timeout`, `unavailable` → повідомлення «спробуйте ще». `is_chat` — евристичний прапор (`core/model_filter.py`); список повний і відсортований, рішення показувати/ховати — на FE. Ключ — транзит/in-memory, **не логується**.
...
- `GET /admin/users/{id}/usage` → `200 { user: AdminUserSummary, events: [{ id, created_at, mode, message, selected_model|null, model_name|null, total_tokens|null, success }], total_requests, total_tokens }` (останні ≤500 подій). **404** якщо невідомий. **PH32 (D-22):** `model_name` = реальна модель ходу (`selected_model` лишається слотом; `null` → FE фолбек на лейбл слота).
...
- `DELETE /admin/users/{id}` → `200 { deleted: true, id }` (PH34). Видаляє акаунт і **всі** його дані (чати/повідомлення, збережені Single-чати, rolling-історію, ledger `usage_events`, BYOK-креденшали, налаштування, сесії — через FK-каскади; RAG-вектори очищаються окремо). Потребує CSRF. **Гарди:** не можна видалити **власний** акаунт (`403`) і **первинного адміна** (`ADMIN_USERNAME`, `403`) — щоб адмін не заблокував себе. **404** якщо невідомий.
...
- `GET /reports/by-model` → `200 { models: [{ model|null, key_fingerprint|null, model_name|null, role:"responder"|"judge", requests, total_tokens, successful }] }` (за спаданням запитів). **PH31 (D-21):** рядки розділено за `(model, key_fingerprint)`. **PH32 (D-22):** групування включає й `model_name` (реальна модель) — той самий слот з власним ключем на різні реальні моделі → окремі рядки; `model_name=null` → FE показує лейбл слота (`model`=слот лишається стабільним групувальником). **PH34 (D-24, B9b):** `role` розрізняє responder-рядок (переможець ходу) і **похідний** judge-рядок доданого (BYOK) судді (`role="judge"`, `model="byok-judge"`, реальна модель у `model_name`, маска у `key_fingerprint`, `total_tokens=0` — не подвоює токени переможця); judge-рядки прихована під `access=app` (суддя own-key за визначенням), видимі під `own`/без фільтра.
- `GET /reports/by-chat` → `200 { chats: [{ chat_id|null, title|null, mode|null, model|null, model_name|null, requests, total_tokens, last_event|null }] }`. `chat_id=null` — група «видалені/ad-hoc» (чат видалено → `chat_id` SET NULL, або хід без чату). LEFT JOIN `chats`. **PH32 (D-22):** `model_name` = репрезентативна реальна модель чату (`func.max`); `null` → FE фолбек на лейбл слота.
...
- `GET /reports/breakdown` (PH28) → `200 { groups: [{ access_key:"app"|"own", requests, total_tokens, models: [{ model|null, key_fingerprint|null, model_name|null, role:"responder"|"judge", requests, total_tokens, chats: [{ chat_id|null, title|null, mode|null, requests, total_tokens }] }] }] }`. Вкладене дерево **ключ доступу → модель → чати** для акордеон-drill-down; зібране в Python з однієї пласкої вибірки (LEFT JOIN chats), сортування за запитами спадно; `chat_id=null` — bucket «видалені/ad-hoc». **PH31 (D-21):** вузол моделі ключований парою `(model, key_fingerprint)`. **PH32 (D-22):** ключ вузла включає й `model_name`; вузол несе `model_name` (реальна модель; `null` → FE фолбек на лейбл слота). Верхній рівень `access_key` (billable) без змін. **PH34 (D-24, B9b):** `role` — `responder` або **похідний** judge-вузол доданого (BYOK) судді, що додається під групу **`own`** (суддя own-key); top-level access-лічильники турів не змінюються.
- `GET /reports/events?cursor=&limit=` → `200 { events: [{ id, created_at, mode, model|null, model_name|null, total_tokens|null, token_estimated, success, billable, key_fingerprint|null, judge_model_name|null, judge_key_fingerprint|null, message, chat_id|null, chat_title|null }], next_cursor|null }`. Keyset-пагінація за `(created_at,id)` спадно; `limit` 1..200 (дефолт 50); `cursor` = `"<iso>|<id>"`. `key_fingerprint` (PH31/D-21) = маска BYOK-ключа ходу або `null`. **PH32 (D-22):** `model_name` = реальна модель ходу (`null` → FE фолбек на лейбл слота). **PH34 (D-24, B9b):** `judge_model_name`/`judge_key_fingerprint` — доданий (BYOK) суддя ходу inline (один рядок на хід; `null` = вбудований/без судді).
- `GET /reports/events.csv` → `200 text/csv` (`Content-Disposition: attachment`), стрім рядками з того самого вікна; колонки: `created_at, mode, model, model_name, key_fingerprint, judge_model_name, judge_key_fingerprint, chat_title, billable, total_tokens, token_estimated, success, message` (PH31/D-21 додав `key_fingerprint`; **PH32/D-22** додає `model_name` після `model`; **PH34/D-24** додає `judge_model_name`/`judge_key_fingerprint` після `key_fingerprint`). CSV-екранування — стандартним `csv`-модулем; великі обсяги не тримаються в памʼяті (keyset-сторінки).
