...
- **PH33 UX-поліш (D-23):** (B2) плашка `.cc-your-model` (токени `--accent`/`--accent-contrast`, як прапор переможця Compare, **без рамки**) у `MainHead`-чипі та картці `SingleModelPicker` — лише для моделі на власному ключі (`byokModelId(slot) !== null`; built-in без плашки); це active-model поверхня → на поточних ключах (D-22). (B4) приглушена світла тема: лише light-гілка `theme/tokens.css` — muted off-white canvas, білі поверхні «плавають» над фоном (глибина як у темній), крихкіші межі; темну тему не чіпано. (B6) `components/keys/ModelCombobox.dedupeModels()` дедуплює discovery-список за нормалізованим `id` на рівні даних. (B3a) `errors.lengthExceeded`/`compare.fail.lengthExceeded` для нового reason-коду. (B3b) `services/{chatApi,compareApi}` передають UI-локаль (`lang`) як `locale`.
...
- **Атрибуція джерела ключа (PH31, D-21):** спільний `components/reports/KeyBadge.tsx` показує, ЧИМ викликана модель — бейдж **«Вбудована»** (`key_fingerprint=null`) або **маска свого ключа** (`перші4••••останні4`, mono). Утиліта `reportUtils.keySource(fp)→{builtin,mask}` (текст бейджа — через `t()` у компоненті). `ByModelTab`/`ActivityLogTab` — окремий стовпець «Ключ»; `BreakdownTab` — бейдж у вузлі моделі (та сама модель built-in vs свій ключ → окремі рядки/вузли, бо бекенд розділяє за `(model, key_fingerprint)`). Звіти суворо per-user — маску бачить лише власник.
- **Правдиве ім'я моделі (PH32, D-22):** єдиний хелпер `utils/models.modelDisplay(slot, model, isByok)` — `isByok ? (model || responderLabel(slot)) : responderLabel(slot)` (built-in → дружня назва слота; власний ключ → реальна модель; фолбек на лейбл слота для legacy-рядків без реальної моделі). Усі **історичні/звітні** поверхні переходять на нього: звіти через `reportUtils.reportModel(row)=modelDisplay(row.model, row.model_name, !!row.key_fingerprint)` (`ByModelTab`/`BreakdownTab`/`ActivityLogTab`); `ByChatTab` — `c.model_name ?? modelLabel(c.model)`; банер переможця `SelectorBanner` бере `winnerModel`/`winnerIsByok` зі **збереженого** ходу (`CompareTurn`: `all_responses[winnerSlot].{model,is_byok}`); картки `CompareColumn`/`CompareFailedCard` — `modelDisplay(provider, model, is_byok)` зі збереженого ходу (прибрано залежність від поточних ключів `useKeys`); `AdminPanel` — `ev.model_name ?? responderLabel(ev.selected_model)`. **Свідома межа (не люк):** композерні/active-model поверхні (`MainHead`/`SingleModelPicker`/`ModelSwitcher`/`sidebarStatus`) лишаються на `byokModelId` (поточні ключі) — вони показують, ЧИМ ти збираєшся слати **зараз**.
- **i18n** — ключі `reports.*` (вкладки/період/KPI/колонки/бейджі/розкладка/ключ доступу/`col.key`+`builtinModel`/назад) у паритеті uk/pl/en. **CSS** — секція «USAGE REPORTS» (`theme/components.css`, `rep-*` на токенах): `.rep-page`/тулбар/вкладки/акордеон/`.rep-keybadge`; responsive 768/430 (спліти 1 колонка, таблиці — горизонтальний скрол усередині `rep-table-wrap`, вкладки — скрол у рядку).
