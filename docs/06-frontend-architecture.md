...
- **Налаштування** (`components/settings/SettingsModal`, стан `store/SettingsContext`): розділи **Критерії суддівства** (`JudgePromptSection`, `services/preferencesApi.getJudgePrompt/putJudgePrompt`) і **API-ключі** (`ApiKeysSection` → `KeysForm`, винесено з `KeysModal`). _Редагуються лише **критерії** (на що зважати судді); механічний каркас — роль/правила/формат/шкала 0–100 — фіксований системно (бекенд `selector_judge` + `$judging_criteria`), користувач його не бачить і не може зламати._ Заглушки — `components/common/ComingSoonModal` (стан `ComingSoonContext`): Профіль/Безпека (Звіти **graduated** у реальний дашборд — PH27/D-18, нижче).
  - **BYOK UX (PH29 / PH29.1, D-19) — Design X (максимальне спрощення):** **вбудовані слоти** (Суддя + дефолтні AI 1/2/3) **не мають поля base URL** — endpoint фіксований (вбудований); рядок = API-ключ + ID моделі + кнопка **«Очистити»** (`keys.clear`), що скидає слот назад до вбудованого. **base URL є лише в кастомних AI 4/5** — `components/keys/BaseUrlSelect` (нативний `<select>` лише з курованого каталогу `utils/byokEndpoints.BYOK_BASE_URLS` = 10 OpenAI-сумісних провайдерів; **без built-in, без груп, без «Власний…»/вільного тексту**; disabled-плейсхолдер «Оберіть ендпоінт…»; fallback-опція для legacy-значення) + «Видалити». Поля мають ⓘ-підказки (`components/common/InfoTip` — a11y popover Esc/click-out/`role="note"`). **Неповний рядок не зберігається:** built-in слот/суддя з ключем без моделі (чи навпаки) бланкується назад до вбудованого (`KeysContext.buildPersistedState`); кастомний без усіх трьох полів не валідується/не зберігається; обидва показують inline-підказку (`keys.incompleteBuiltin`/`keys.incompleteCustom`). **Свідомо прибрано** (за рішенням власника): PH22-оверайд endpoint'у дефолтних слотів і base URL судді з UI (для чужого провайдера — AI 4/5). `JudgeKey.baseUrl` лишається в payload-контракті (бекенд приймає; UI його не виставляє). Безпека D-12 збережена: ключі лише в `sessionStorage` + транзит, не в БД/логах.
