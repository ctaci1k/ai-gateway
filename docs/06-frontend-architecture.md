...
- **Налаштування** (`components/settings/SettingsModal`, стан `store/SettingsContext`): розділи **Критерії суддівства** (`JudgePromptSection`, `services/preferencesApi.getJudgePrompt/putJudgePrompt`) і **API-ключі** (`ApiKeysSection` → `KeysForm`, винесено з `KeysModal`). _Редагуються лише **критерії** (на що зважати судді); механічний каркас — роль/правила/формат/шкала 0–100 — фіксований системно (бекенд `selector_judge` + `$judging_criteria`), користувач його не бачить і не може зламати._ Заглушки — `components/common/ComingSoonModal` (стан `ComingSoonContext`): Профіль/Безпека (Звіти **graduated** у реальний дашборд — PH27/D-18, нижче).
  - **BYOK base URL — Select, не вільний текст (PH29/D-19):** кожен рядок `KeysForm` має `components/keys/BaseUrlSelect` — нативний `<select>` з курованим каталогом `utils/byokEndpoints.BYOK_BASE_URLS` (optgroup «Вбудовані»/«Сумісні») + «Власний…» (показує текстове поле). Дефолтні слоти й суддя мають опцію «Типовий ендпоінт» (→ порожній `baseUrl`, бекенд бере фіксований); кастомні AI 4/5 — без неї. **Суддя теж має base URL** (`JudgeKey.baseUrl`, дефолт «Вбудований суддя (Groq)») і кнопку **«Очистити»** (скидає key/model/baseUrl → вбудований Qwen на Groq); кастомні — **«Видалити»**. Поля мають ⓘ-підказки (`components/common/InfoTip` — a11y popover Esc/click-out/`role="note"`): де взяти ключ, що таке ID моделі, парність із base URL. Безпека D-12 збережена: ключі лише в `sessionStorage` + транзит, не в БД/логах.
