...
## Звіти про використання (PH27/D-18)

Реальний self-service дашборд історії акаунта (замінює заглушку `ComingSoonModal("reports")`). Кнопка «Звіти» (`AccountMenu`) → `store/ReportsContext.open()`; модалку змонтовано в `MainLayout`. Дані — лише через `services/reportsApi.ts` (без `fetch` у компонентах), типи — `types/api.ts` (`ReportSummary/ModelUsage/ChatUsage/TimeseriesPoint/ReportEvent`).

- **`components/reports/ReportsModal.tsx`** — модалка на рамці `settings-dialog` (a11y: role=dialog/aria-modal/focus-trap/Esc/restore). Зверху **фільтр періоду** (`rep-rangebar`: 24г/7д/30д/усе/власний + два date-inputs). Ліва навігація вкладок: **Огляд / За моделями / За чатами / Журнал**. Зміна періоду **remount-ить** активну вкладку через `key=rangeKey` → mount-once fetch (патерн `AdminPanel`, без синхронного setState в ефекті — `react-hooks/set-state-in-effect`).
- **`OverviewTab`** — KPI-картки (запити, токени з «~»+бейдж «оцінка», чати, сер./день, success rate); **`MiniChart.tsx`** (легкий inline-SVG: бари=запити + лінія=токени на токенах теми, `role="img"`+`aria-label`, без важких залежностей — D-18/п.7); спліт-бари Single/Compare і billable/own-key.
- **`ByModelTab`/`ByChatTab`** — таблиці (`scope`/`th`); за чатами рядок клікабельний (Enter/Space) → `useChatMode.setMode` + `useChats.selectChat` + закриває модалку; видалені/ad-hoc — окремий рядок; відносний час — `utils/relativeTime`. Назви моделей — `utils/models` (BYOK → `model_id`).
- **`ActivityLogTab`** — keyset-пагінований лог (час/режим/модель/чат/сніпет/токени/статус/own-key-бейдж) + «Показати ще» (`getEvents(cursor)`) + «Експорт CSV» (`eventsCsvUrl`, серверний стрім). Стани loading/empty/error — спільний `RepState`.
- **i18n** — ~50 ключів `reports.*` у паритеті uk/pl/en. **CSS** — секція «USAGE REPORTS» (`theme/components.css`, `rep-*` на токенах); responsive `@media(max-width:768px)` (спліти 1 колонка, таблиці — горизонтальний скрол усередині `rep-table-wrap`).
