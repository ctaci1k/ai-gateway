...
- **Робастний парс контенту (PH33/B3a, D-23).** `OpenAICompatibleProvider._message_text()` читає видиму відповідь із основного `content`, а за порожнім — із запасних reasoning-полів (`reasoning_content`/`reasoning`) у `generate_full` і в стрімі (буфер reasoning, якщо `content` так і не прийшов). Це усуває «глухий empty» на reasoning-моделях типу Mistral-magistral. Якщо видимого контенту немає й після фолбеків — `_empty_error(finish_reason)` дає чітку причину: `finish_reason=length` → новий reason-код **`length_exceeded`** (output truncated), інакше `empty_response`. `classify_provider_failure` мапить нові формулювання; наявні коди не зламані.
- **Мова відповіді (PH33/B3b, D-23).** `core.prompts.with_language_directive(message, locale)` додає директиву мови у **responder_message** (відповідачі — і Compare через orchestrator, і Single-стрім через `routes/chat.py`): відповідач відповідає мовою повідомлення, `locale` (з `ChatRequest.locale`) — фолбек. Суддя/ембеддинги не зачіпаються (суддя отримує оригінальне `message`).
...
4. **Критерії авторитетні (PH33/B5, D-23):** каркас `selector_judge` (`prompts.yaml` v7) має блок «SCORING DISCIPLINE» — бали МУСЯТЬ відображати `$judging_criteria`, явно кращий за критеріями → суттєво вищий бал (не near-tie), а персоналізаційний хінт — лише тайбрейкер. За наявності `judge_prompt_override` (явні критерії користувача) пост-зваження **вимикається** повністю: `apply_preference_weighting(criteria_override=True)` повертає вибір судді без змін (`suppressed_by_criteria`), щоб нудж не перебивав явні критерії.
